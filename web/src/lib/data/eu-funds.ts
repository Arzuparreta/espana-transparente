...
import { unstable_cache, HOUR, PAGE_SIZE, throwDataError, type EuFundRow, type EuFundsSummary } from "./shared"
...
export const getEuFundsPage = unstable_cache(
  async (page: number) => {
    ...
    const { data, count, error } = await supabase
      ...
      .range(from, to)
    throwDataError(error, "EU funds page")
...
export const getEuFundsSummary = unstable_cache(
  async () => {
    const { data, error } = await supabase.from("v_eu_funds_summary").select("*").single()
    throwDataError(error, "EU funds summary")
...
export const getEuFundBySlug = unstable_cache(
  async (slug: string) => {
    const { data, error } = await supabase
      ...
      .limit(1)
      .maybeSingle()
    throwDataError(error, "EU fund detail")
