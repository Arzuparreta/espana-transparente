...
import {
  ...
  PAGE_SIZE,
  throwDataError,
  type ContractResponsibilityRow,
} from "./shared"
    ...
    const responsibilities =
      contractIds.length > 0
        ? await supabase
            ...
            .select(
              "contract_id, person_name, politician_id, official_id, ministry, government, political_party, administration_level, territory_name, match_method",
            )
            .in("contract_id", contractIds)
        : { data: [] }
    throwDataError(
      "error" in responsibilities ? responsibilities.error : null,
      "contract responsibilities",
    )

    const responsibleByContract = new Map(
      ((responsibilities.data ?? []) as ContractResponsibilityRow[]).map(
        (row) => [
          row.contract_id,
          {
...
export const getContractPageFiltered = unstable_cache(
  async (
    ...
    role: "awarding" | "recipient" | "all" = "all",
  ) => {
    const from = (page - 1) * PAGE_SIZE.contracts
    const to = from + PAGE_SIZE.contracts - 1
    let query = supabase
    ...
    if (organization) {
      if (role === "awarding")
        query = query.eq("awarding_body_organization_id", organization)
      else if (role === "recipient")
        query = query.eq("contractor_organization_id", organization)
      else
        query = query.or(
          `awarding_body_organization_id.eq.${organization},contractor_organization_id.eq.${organization}`,
        )
    }
    if (ministry) query = query.eq("ministry_normalized", ministry)
    if (level && VALID_ADMIN_LEVELS.has(level))
      query = query.eq("administration_level", level)
  ...
  async (id: string) => {
    const [contract, responsibility] = await Promise.all([
      ...
      supabase
        .from("v_contract_responsibility")
        .select(
          "person_name, politician_id, official_id, ministry, government, political_party, administration_level, territory_name",
        )
        .eq("contract_id", id)
        .maybeSingle(),
    ])
