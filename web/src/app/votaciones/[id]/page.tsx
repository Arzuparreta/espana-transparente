  ...
  return (
    <div className="ui-page">
      <ContextTrail
        ...
        related={[
          initiative
            ? { href: `/iniciativas/${initiative.id}`, label: "Iniciativa", meta: initiative.type ?? undefined }
            : null,
          divergences.length > 0
            ...
            : null,
          sorted.length > 0
            ? { href: `#resultado-por-grupo`, label: "Resultado por grupo", meta: String(sorted.length) }
            : null,
        ...
        hero={
          <section className="flex flex-col gap-3 border-b border-border pb-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="text-xs">{chamberLabel}</Badge>
              <span className="font-mono text-xs text-muted-foreground">{dateStr}</span>
            </div>
            {tipo ? (
              <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{tipo}</p>
            ) : null}
            ...
            {session.initiative_number ? (
              <p className="font-mono text-xs text-muted-foreground">Exp. {session.initiative_number}</p>
            ) : null}
          </section>
        }
      >
        {initiative ? (
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Iniciativa origen</span>
            <EntityLink kind="initiative" id={initiative.id} className="min-w-0 truncate font-medium underline-offset-2 hover:underline">
          ...
          >
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {VOTE_ORDER.map((vote) => (
                <div key={vote} className="min-w-0">
                  <div className="font-mono text-[11px] uppercase tracking-[0.1em] text-muted-foreground">{vote}</div>
                  <div
                    data-value
            ...
            <div className="mt-4 flex h-2.5 overflow-hidden rounded-[2px] bg-muted" aria-hidden="true">
              {VOTE_ORDER.filter((vote) => (totals[vote] ?? 0) > 0).map((vote) => (
                <div
                  key={vote}
                  style={{ width: `${((totals[vote] ?? 0) / totalRecorded) * 100}%`, backgroundColor: getVoteColor(vote) }}
                />
              ))}
            </div>
          </RecordSection>
        ...
        {divergences.length > 0 ? (
          <section id="divergencias" className="scroll-mt-24 rounded-[2px] border border-accent/35 bg-accent/[0.06] p-4">
            <div className="mb-3 flex min-w-0 items-center justify-between gap-3">
              <h2 className="font-display text-lg font-black uppercase tracking-[-0.02em]">Divergencias relevantes</h2>
              <ExceptionBadge count={divergences.length} />
            </div>
            <div className="space-y-2">
              {divergences.map((divergence, index) => (
                <div key={index} className="flex flex-wrap items-center gap-2 border-l-2 border-accent pl-3 text-sm">
                  ...
                  <span className="text-xs">
                    ...
                    <b style={{ color: getVoteColor(divergence.partyVoted) }}>{divergence.partyVoted}</b> (su grupo)
                  </span>
                </div>
              ))}
            </div>
        ...
        <RecordSection
          id="resultado-por-grupo"
          title="Resultado por grupo"
          eyebrow="Barra proporcional al tamaño del grupo"
        >
          <div className="border-t border-border/60">
            {sorted.map(([acronym, group]) => (
              <div
                key={acronym}
                className="flex flex-col gap-2 border-b border-border/60 py-3 sm:flex-row sm:items-center sm:gap-4"
              >
                <div className="flex min-w-0 items-center justify-between gap-3 sm:w-32 sm:shrink-0">
                  ...
                  <span className="shrink-0 font-mono text-xs tabular-nums text-muted-foreground">{group.total}</span>
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  >
                    {Object.entries(group.votes)
                      .sort((a, b) => b[1] - a[1])
                      .map(([vote, count]) => (
                        <div key={vote} style={{ width: `${(count / group.total) * 100}%`, backgroundColor: getVoteColor(vote) }} />
                      ))}
                  </div>
                </div>
                  ...
                  {Object.entries(group.votes)
                    ...
                    .map(([vote, count]) => (
                      <span key={vote} className="flex items-center gap-1.5 text-muted-foreground">
                        <span className="h-2 w-2 shrink-0" style={{ backgroundColor: getVoteColor(vote) }} />
                        {count}
                      </span>
                    ))}
                </div>
        ...
        <details className="text-sm">
          ...
          <div className="mt-3 max-h-96 overflow-y-auto border-t border-border pt-2">
