...
interface Row {
  ...
  value: number | string
  unit: string | null
  raw_data?: { retrieved_at?: string; flags?: string[] }
...
export default async function IndicadorPage({ params }: PageProps) {
  ...
  const isFiscal = code in FISCAL_SERIES
  ...
  const stats = [
    { label: "Periodo", value: latest.period, hint: "Última observación" },
    {
      ...
      hint: "Respecto al dato anterior",
    },
  ...
  const ipcSubgroups = code === "IPC" ? await getIpcSubgroupSeries() : null
  ...
  const latestSalary = salaryPoints?.length
    ? Number(
        [...salaryPoints].sort((a, b) => a.period.localeCompare(b.period))[
  ...
  return (
    <div className="ui-page">
      <ContextTrail
        section={{
          href: isFiscal ? "/cuentas-publicas" : "/indicadores",
          label: isFiscal ? "Cuentas públicas" : "Series económicas",
        }}
        current={name}
        meta={code}
        fallbackHref={isFiscal ? "/cuentas-publicas" : "/indicadores"}
        fallbackLabel={
          isFiscal ? "Volver a Cuentas públicas" : "Volver a Series"
        }
        related={[{ href: "/estado-datos", label: "Estado de los datos" }]}
      />
      {isFiscal && (
        <SourceFootnote
          sourceLabel="Eurostat · AA. PP. S13 · millones de euros"
          lastChecked={latest.raw_data?.retrieved_at}
          sourceHref={
            code === "DEUDA_PUBLICA"
          ...
          coverageLabel={`${rangeStart}–${rangeEnd}`}
        />
      )}
          ...
          <div className="flex flex-col gap-3 rounded-[2px] border border-border bg-background/60 px-3 py-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="shrink-0 rounded-[2px] border border-border bg-card px-3 py-2 sm:text-right">
              ...
              <div className="mt-1 flex flex-wrap items-end gap-2 sm:justify-end">
                <span
                  data-value
                  className="font-mono text-4xl font-medium tracking-tight sm:text-5xl"
                >
          ...
          <div className="grid flex-1 min-h-0 gap-3 lg:grid-cols-[minmax(0,1fr)_260px]">
            ...
            <aside className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              {stats.map((stat) => (
                <div
                  key={stat.label}
                  className="rounded-[2px] border border-border bg-background/60 px-3 py-3"
                >
                  <div className="font-mono text-xs uppercase tracking-[0.1em] text-muted-foreground">
                    {stat.label}
                  </div>
                  <div
                    data-value
                    className="mt-1 break-words font-mono text-3xl font-medium tracking-tight"
                  >
                    {stat.value}
                  </div>
      ...
      {ipcSeries && ipcSeries.length > 1 && (
        <>
          ...
          {code === "IPC" && <PurchasingPowerCalculator series={ipcSeries} />}
        </>
      )}
