...
export function DistorsionTabs({ dates, activeDate, withVotesPerSeat, maxVotesPerSeat, minVotesPerSeat, provinces }: DistorsionTabsProps) {
  return (
                ...
                <CardContent className="space-y-3">
                  {withVotesPerSeat.map((result) => (
                    <div key={result.party} className="flex items-center gap-2 text-xs sm:gap-3 sm:text-sm">
                      <div className="w-16 shrink-0 text-right font-medium sm:w-20" style={{ color: result.color }}>
                  ...
                  <div className="mt-4 border-t pt-2 text-xs text-muted-foreground">
                    Un escaño de {withVotesPerSeat[0]?.partyShortName} (&quot;cuesta&quot;{" "}
                    {withVotesPerSeat[0]?.votesPerSeat.toLocaleString()} votos) vale igual que uno
                    de {withVotesPerSeat[withVotesPerSeat.length - 1]?.partyShortName} (
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  {withVotesPerSeat.map((result) => {
                    ...
                    const cls =
                      diff > 1 ? "text-green-600 dark:text-green-400" : diff < -0.5 ? "text-red-600 dark:text-red-400" : "text-muted-foreground"
                    return (
                      <div key={result.party} className="flex flex-wrap items-center gap-1.5 text-xs sm:gap-2 sm:text-sm">
                        <span className="w-12 text-right font-medium sm:w-16" style={{ color: result.color }}>
                          {result.partyShortName}
                        </span>
