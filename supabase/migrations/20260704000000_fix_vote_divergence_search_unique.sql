-- Dedupe vote_divergence search rows when multiple voting_sessions share title+date.
-- Also re-applies refresh_vote_divergences_cache DISTINCT ON for DBs that skipped 20260703000000.
...
BEGIN
  ...
  IF p_entity_type IS NULL OR p_entity_type = 'vote_divergence' THEN
    ...
    INSERT INTO search_documents (
      ...
      corpus_version, updated_at)
    WITH src AS (
      SELECT DISTINCT ON (d.full_name, d.initiative, d.date)
      ...
      ORDER BY d.full_name, d.initiative, d.date, (vs.id IS NOT NULL) DESC, vs.id NULLS LAST
    )
