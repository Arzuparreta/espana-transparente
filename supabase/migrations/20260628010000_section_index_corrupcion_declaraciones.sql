-- Add corrupcion and declaraciones counts to the home atlas section index.
-- Both sections exist in the nav but were absent from get_section_index(),
-- making them invisible on the home page "Qué hay aquí" atlas.
...
AS $$
  ...
  UNION ALL
  SELECT 'corrupcion',
         (SELECT COUNT(*)::bigint FROM corruption_cases),
         (SELECT MAX(last_verified_at)::date FROM corruption_cases)
  UNION ALL
