-- Fix structural search-route bug: every entity must produce a URL that uniquely
-- identifies it. Previously eu_fund, revolving_door, the government_position/
-- institution fallbacks, budget_program and source_document all routed to the
-- bare index page, so every result of those types landed on the same page.
--
-- Changes vs corpus v2 (20260523010000):
--   eu_fund          -> '/fondos-ue/<slug>'             (slug = last Kohesio URI segment)
--   revolving_door   -> '/puertas-giratorias/<uuid>'
--   government_position: source from v_gobierno_actual so id matches /ministerios/[id]
--                     route -> '/ministerios/<id>'      (existing detail page)
--   institution      -> '/instituciones/<id>'           (new detail page; redirects to
--                                                        /diputados/<id> when politician_id exists)
--   budget_program   -> '/presupuestos/<section>/<program>'  (new detail page)
--   source_document  -> sd.source_url                   (NULL rows filtered out)
--
-- The full corpus is rebuilt at the end of the migration so stale routes are
-- evicted in one shot.
...
-- Drop stale corpus so the new function rebuilds with the fixed routes.
