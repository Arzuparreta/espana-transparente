...
- `senado/` — Senate scrapers (active senators + memberships, `chamber='senate'`). Same 1.5s rate-limit policy as Congreso. `senado.es` is behind Akamai: from a datacenter IP it 403s any request that lacks the `Sec-Fetch-*` navigation headers, and it re-blocks the IP for a while under bursts. All Senate fetches must go through `common.http_headers.curl_header_args()` and keep the delay. Residential IPs are not challenged, so **this class of breakage does not reproduce locally**.
...
- `kohesio/` — EU fund beneficiaries for Spain from the Kohesio API (ESIF 2014-2027); the API only serves `offset=0`, so each run ingests up to ~30K of ~80K records. **The EC edge 403s datacenter traffic on every path** — verified 2026-08-20 from both the VPS runner and a GitHub-hosted runner, while the same requests succeed from a residential connection. Switching to a source the VPS can reach was investigated and rejected (2026-08-22): on `cohesiondata.ec.europa.eu`, all 139,395 Spanish rows of the Kohesio projects dataset have a null beneficiary, and the ERDF beneficiaries dataset pseudonymises names (`benef_name = "1"`) with a null EU allocation for Spain — switching would drop the named-recipient list entirely.
  So the fetch runs off-VPS: `scripts/push-kohesio-payload.sh` fetches from a residential machine and pushes the payload to the VPS over Tailscale (a systemd **user** timer on `desktop-ruben` runs it daily; `Linger=yes`, so it needs the machine powered on, not logged in). `weekly-core` ingests it via the `KOHESIO_PAYLOAD_PATH` repo variable. The payload carries a `fetched_at` stamp and `--from-file` refuses anything older than `--payload-max-age-days` (14), so a dead fetcher fails the weekly run loudly instead of re-ingesting frozen data.
