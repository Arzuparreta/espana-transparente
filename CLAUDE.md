...
- `kohesio/` — EU fund beneficiaries for Spain from the Kohesio API (ESIF 2014-2027); the API only serves `offset=0`, so each run ingests up to ~30K of ~80K records. **The EC edge 403s datacenter traffic on every path** — verified 2026-08-20 from both the VPS runner and a GitHub-hosted runner, while the same requests succeed from a residential connection. There is no automated path to this API from any runner we control, so `eu_funds` is frozen at the last successful run. To refresh: run `--fetch-only <path>` from an unblocked machine and ingest with `--from-file`, or set the `KOHESIO_PAYLOAD_PATH` repo variable to a payload on the VPS. Restoring automation needs either a non-datacenter egress or a different source.
