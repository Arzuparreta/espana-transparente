...
- `kohesio/` — EU fund beneficiaries for Spain from the Kohesio API (ESIF 2014-2027); the API only serves `offset=0`, so each run ingests up to ~30K of ~80K records. The EC edge returns a bare 403 to the VPS runner's IP on every path, so the weekly run **fetches on a GitHub-hosted runner** (`etl-kohesio-fetch` → artifact) and ingests with `--from-file`; a direct fetch still works from any unblocked host.
